...
class MockEngine:
    ...
    def normalize(raw: dict[str, Any]) -> dict[str, Any]:
        """Convert raw engine output to standardized PageResult JSONB structure.

        Validates the output through Pydantic models to ensure schema compliance.
        ...
        Returns:
            ...
            - pages: list of PageResult dicts (validated schema)
            - engine_id: engine identifier
            - engine_version: version string
            - config_snapshot: config used for the run
        """
        pages_data = raw.get("raw_pages", [])
        pages = [PageResult(**p) for p in pages_data]
